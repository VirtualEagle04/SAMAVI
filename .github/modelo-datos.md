...
## 2. Resumen de tablas (18)

| # | Tabla | Tipo | PK |
|---|-------|------|----|
| 1 | SOCIO | Fuerte | id |
| 2 | GALPON | Fuerte | id |
| 3 | INVIERTE_EN | Unión M:N | (id_socio, id_galpon) |
...
| 10 | PEDIDO | Fuerte | id |
| 11 | DETALLE_PEDIDO | Débil | (id_pedido, codigo_categoria_peso) |
| 12 | VENTA | Fuerte | id |
| 13 | DETALLE_VENTA | Débil | (id_venta, codigo_categoria_peso) |
| 14 | ROL | Fuerte | id |
| 15 | PERMISO | Fuerte | id |
| 16 | ROL_PERMISO | Unión M:N | (id_rol, id_permiso) |
| 17 | USUARIO | Fuerte | id |
| 18 | AUDITORIA | Fuerte | id |
...
Valores esperados según levantamiento: Yumbo (78g+, peso_max_g NULL), Extra (67-77g), AA (60-66g), A (53-59g), B (46-52g), C (45g), P (menos de 45g), Q (Quebrados, no tiene rango de peso, peso_min_g y peso_max_g NULL).
...
|---|---|---|---|---|---|
| 1 | INVIERTE_EN | id_socio | SOCIO | N a 1 | CASCADE |
| 2 | INVIERTE_EN | id_galpon | GALPON | N a 1 | CASCADE |
...
1. **Prorrateo (cálculo semanal de reparto de ganancias):**
   ...
   - Balance = ventas asignadas - gastos. La distribución a cada socio = balance * porcentaje_inversion (de INVIERTE_EN).
   - Esta lógica de cálculo debe implementarse en la capa de servicio/aplicación, no en el esquema; el esquema solo almacena los datos base necesarios (CIERRE_DIARIO, DETALLE_VENTA, PRECIO, INVIERTE_EN).
...
3. **Auditoría obligatoria:** toda escritura (INSERT/UPDATE/DELETE) sobre tablas transaccionales (PEDIDO, VENTA, DETALLE_PEDIDO, DETALLE_VENTA, PRECIO, INVIERTE_EN, USUARIO, ROL_PERMISO) debe generar una fila en AUDITORIA. Se recomienda implementarlo vía trigger de base de datos o middleware de aplicación (a decidir con el equipo; no se define aquí para no imponer una arquitectura).
...
5. **MQTT y ESP32:** el ESP32 NO forma parte del modelo de datos. Solo envía mensajes MQTT (topic `saman/conteo`) que el backend traduce en filas de LOG_CONTEO. La consolidación diaria (CIERRE_DIARIO) es un proceso de agregación posterior, ejecutado por el backend.
...
- `CATEGORIA_PESO.codigo`: `Y`, `EX`, `AA`, `A`, `B`, `C`, `Q`
